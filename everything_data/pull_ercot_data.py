import pandas as pd
from everything_data.config import (
    ...
    if time_columns is None:
        ...
        for col in df.columns:
            if any(
                ...
                for keyword in ["time", "date", "datetime", "hour", "timestamp"]
            ):
...
def download_and_extract_zip(url: str) -> pd.DataFrame:
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as zip_file:
            ...
            excel_files = [
                f for f in zip_file.namelist() if f.endswith((".xlsx", ".xls"))
            ]
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as zip_file:
            ...
            if all_sheets_data:
                ...
                result_df = pd.concat(
                    all_sheets_data, axis=0, ignore_index=True, sort=False
                )
                # Fix ERCOT time format issues
